...
export function LineChart({ results }: LineChartProps) {
    ...
    return (
      <div className="card p-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-2xl font-semibold text-content">Growth Over Time</h2>
        </div>
        <p className="text-content-muted">Run a simulation to see the chart.</p>
      </div>
    );
  }
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-surface-card border border-th-border-light rounded-xl shadow-xl p-4">
          <p className="font-semibold text-content mb-3">Year {label}</p>
          {payload.map((entry: any, index: number) => (
            ...
            >
              ...
              <span className="font-semibold text-content">{formatCurrency(entry.value)}</span>
            </p>
          ))}
        </div>
      );
  ...
  return (
    <div className="card p-8">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h2 className="text-2xl font-semibold text-content">Account Balance Over Time</h2>
      </div>
      ...
      <ResponsiveContainer width="100%" height={400}>
        <RechartsLineChart data={chartData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
          ...
          <XAxis
            dataKey="year"
            label={{ value: 'Year', position: 'insideBottom', offset: -5, fill: chartColors.axis, fontSize: 12 }}
            stroke={chartColors.axis}
            tick={{ fill: chartColors.axis }}
          />
          <YAxis
            tickFormatter={formatCurrency}
            label={{ value: 'Balance (\u20AC)', angle: -90, position: 'insideLeft', fill: chartColors.axis, fontSize: 12 }}
            stroke={chartColors.axis}
            tick={{ fill: chartColors.axis }}
          />
